...
speechModule::speechModule(std::string clientId, std::string ip_port)
{
  	...
  	//printf("In SpeechModule Atul");
        ...
        stub_ = (TextToSpeech::NewStub(grpc::CreateChannel(ip_port, grpc::InsecureChannelCredentials()/*credentials*/)));
	 printf("In SpeechModule Atul");
}
...
extern "C"
{
	...
	static speechModule *ai_speech = NULL;
        int z = 0;
        void cppwrapperFuc(char *id, char* url)
        {
//		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Api test 1\n");
                std::string clientId(id);
		std::string URL(url);
                if (ai_speech == NULL)
                {
//			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Api test 2\n");
